...
{
   MY_LOGD(("hdhomerun_sock_create_udp(): posix ENTER"));
	/* Create socket. */
	hdhomerun_sock_t sock = (hdhomerun_sock_t)socket(AF_INET, SOCK_DGRAM, 0);
	if (sock == -1) {
      MY_LOGD(("hdhomerun_sock_create_udp(): socket creation failed"));
	...
	if (fcntl(sock, F_SETFL, O_NONBLOCK) != 0) {
		close(sock);
      MY_LOGD(("hdhomerun_sock_create_udp(): set non-blocking failed"));
		return HDHOMERUN_SOCK_INVALID;
	}
