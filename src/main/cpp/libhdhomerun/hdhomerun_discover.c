...
struct hdhomerun_discover_t *hdhomerun_discover_create(struct hdhomerun_debug_t *dbg)
{
	...
	if (!hdhomerun_discover_sock_add(ds, 0, 0)) {
		free(ds);
		MY_LOGD("hdhomerun_discover_create(): failed to add a socket");
		return NULL;
	}
...
{
	int index;
   MY_LOGD("hdhomerun_discover_find_in_list(): Looking for IP:%s ID:%X, %d in results list",
         ipAddressToString(lookup->ip_addr),
         lookup->device_id,
         count);
	for (index = 0; index < count; index++) {
...
{
   MY_LOGD("hdhomerun_discover_find_devices(): ENTER");
	...
	for (attempt = 0; attempt < 2; attempt++) {
		...
		while (1) {
			...
			if (!hdhomerun_discover_recv(ds, result)) {
               ...
               MY_LOGD("hdhomerun_discover_find_devices(): Discovery Timeout");
					break;
				}
				msleep_approx(16);
				continue;
            ...
            MY_LOGD("hdhomerun_discover_find_devices(): Already in list IP:%s, ID:%X",
                  ipAddressToString(result->ip_addr),
                  result->device_id);
				continue;
			}
         
         MY_LOGD("hdhomerun_discover_find_devices(): Found device IP:%s ID:%X",
                  ipAddressToString(result->ip_addr),
                  result->device_id);
   ...
   MY_LOGD("hdhomerun_discover_find_devices(): Returning %d",count);
	return count;
}
...
{
   MY_LOGD("hdhomerun_discover_find_devices_custom(): ENTER");
	if (hdhomerun_discover_is_ip_multicast(target_ip)) {
      MY_LOGD(("hdhomerun_discover_find_devices_custom(): Multicast IP error"));
		return 0;
	}
   ...
   MY_LOGD("hdhomerun_discover_find_devices_custom(): returning %d",ret);
	return ret;
}
...
char* ipAddressToString(uint32_t ipAddr)
{
   static char string[16]; // 15 bits for address and dots, 1 for null terminator
   int size = sizeof(string);

   size -= snprintf(&string[0], size, "%d.", (ipAddr & 0xFF000000) >> 24);
   size -= snprintf(&string[sizeof(string)-size], size, "%d.", (ipAddr & 0x00FF0000) >> 16);
   size -= snprintf(&string[sizeof(string)-size], size, "%d.", (ipAddr & 0x0000FF00) >> 8);
   snprintf(&string[sizeof(string)-size], size, "%d",  (ipAddr & 0x000000FF));
